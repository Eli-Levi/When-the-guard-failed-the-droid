...
def random_forest(num_of_features, features, group, group_mani, report, X_train, X_test, X_test_manipulated, y_train, y_test, y_test_manipulated):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = [executor.submit(algorithms.random_forest_, X_train, y_train, X_test, y_test, X_test_manipulated, 128, criteria,  min_samples_split, features)
                   for criteria in ['entropy', 'gini'] for min_samples_split in [3]]
